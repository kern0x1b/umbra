...
namespace std {
template<>
struct less<Umbra::A64::LocationDescriptor> {
    bool operator()(const Umbra::A64::LocationDescriptor& x, const Umbra::A64::LocationDescriptor& y) const noexcept {
        return x.UniqueHash() < y.UniqueHash();
    }
};
template<>
struct hash<Umbra::A64::LocationDescriptor> {
    size_t operator()(const Umbra::A64::LocationDescriptor& x) const noexcept {
...
template<>
struct fmt::formatter<Umbra::A64::LocationDescriptor> : fmt::formatter<std::string> {
    template<typename FormatContext>
    auto format(Umbra::A64::LocationDescriptor descriptor, FormatContext& ctx) const {
        return formatter<std::string>::format(Umbra::A64::ToString(descriptor), ctx);
    }
};
