...
template<>
struct fmt::formatter<Umbra::A64::Reg> : fmt::formatter<std::string> {
    template<typename FormatContext>
    auto format(Umbra::A64::Reg reg, FormatContext& ctx) const {
        return formatter<std::string>::format(Umbra::A64::RegToString(reg), ctx);
    }
};

template<>
struct fmt::formatter<Umbra::A64::Vec> : fmt::formatter<std::string> {
    template<typename FormatContext>
    auto format(Umbra::A64::Vec vec, FormatContext& ctx) const {
        return formatter<std::string>::format(Umbra::A64::VecToString(vec), ctx);
    }
};
