...
#include "umbra/interface/optimization_flags.h"
...
struct UserConfig {
    ...
    /// The intention behind this is to allow users of Umbra to pack attributes in the
    ...
    /// page table. If true, Umbra will silently mirror page_table's address space. If
    ...
    /// Determines what happens if the guest accesses an entry that is off the end of the
    /// fastmem arena. If true, Umbra will silently mirror fastmem's address space. If
    ...
    /// writes. On x64, umbra currently relies on x64 cmpxchg semantics which may not
