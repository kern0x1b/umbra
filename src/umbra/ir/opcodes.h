...
template<>
struct fmt::formatter<Umbra::IR::Opcode> : fmt::formatter<std::string> {
    template<typename FormatContext>
    auto format(Umbra::IR::Opcode op, FormatContext& ctx) const {
        return formatter<std::string>::format(Umbra::IR::GetNameOf(op), ctx);
    }
};
