...
template<>
struct fmt::formatter<Umbra::IR::Type> : fmt::formatter<std::string> {
    template<typename FormatContext>
    auto format(Umbra::IR::Type type, FormatContext& ctx) const {
        return formatter<std::string>::format(Umbra::IR::GetNameOf(type), ctx);
    }
};
