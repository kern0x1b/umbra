...
SigHandler::SigHandler() {
    ...
    if (sigaltstack(&signal_stack, nullptr) != 0) {
        fmt::print(stderr, "umbra: POSIX SigHandler: init failure at sigaltstack\n");
    ...
    if (sigaction(SIGSEGV, &sa, &old_sa_segv) != 0) {
        fmt::print(stderr, "umbra: POSIX SigHandler: could not set SIGSEGV handler\n");
        supports_fast_mem = false;
        return;
    }
#ifdef __APPLE__
    if (sigaction(SIGBUS, &sa, &old_sa_bus) != 0) {
        fmt::print(stderr, "umbra: POSIX SigHandler: could not set SIGBUS handler\n");
        supports_fast_mem = false;
        return;
    }
#endif
