...
#    include "umbra/backend/x64/mig/mach_exc_server.h"

#    define THREAD_STATE x86_THREAD_STATE64
#    define THREAD_STATE_COUNT x86_THREAD_STATE64_COUNT

using umbra_thread_state_t = x86_thread_state64_t;

#elif defined(MCL_ARCHITECTURE_ARM64)

#    include <oaknut/code_block.hpp>
#    define mig_external extern "C"
#    include "umbra/backend/arm64/mig/mach_exc_server.h"

#    define THREAD_STATE ARM_THREAD_STATE64
#    define THREAD_STATE_COUNT ARM_THREAD_STATE64_COUNT

using umbra_thread_state_t = arm_thread_state64_t;
...
public:
    ...
    kern_return_t HandleRequest(umbra_thread_state_t* thread_state);
...
#define KCHECK(x) ASSERT_MSG((x) == KERN_SUCCESS, "umbra: macOS MachHandler: init failure at {}", #x)
...
void MachHandler::MessagePump() {
    ...
    while (true) {
        ...
        if (mr != MACH_MSG_SUCCESS) {
            fmt::print(stderr, "umbra: macOS MachHandler: Failed to receive mach message. error: {:#08x} ({})\n", mr, mach_error_string(mr));
            return;
        }

        if (!mach_exc_server(&request.head, &reply.head)) {
            fmt::print(stderr, "umbra: macOS MachHandler: Unexpected mach message\n");
            return;
        }

        mr = mach_msg(&reply.head, MACH_SEND_MSG, reply.head.msgh_size, 0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
        if (mr != MACH_MSG_SUCCESS) {
            fmt::print(stderr, "umbra: macOS MachHandler: Failed to send mach message. error: {:#08x} ({})\n", mr, mach_error_string(mr));
            return;
        }
    }
}
...
mig_external kern_return_t catch_mach_exception_raise(mach_port_t, mach_port_t, mach_port_t, exception_type_t, mach_exception_data_t, mach_msg_type_number_t) {
    fmt::print(stderr, "umbra: Unexpected mach message: mach_exception_raise\n");
    return KERN_FAILURE;
}
...
mig_external kern_return_t catch_mach_exception_raise_state_identity(mach_port_t, mach_port_t, mach_port_t, exception_type_t, mach_exception_data_t, mach_msg_type_number_t, int*, thread_state_t, mach_msg_type_number_t, thread_state_t, mach_msg_type_number_t*) {
    fmt::print(stderr, "umbra: Unexpected mach message: mach_exception_raise_state_identity\n");
    return KERN_FAILURE;
}
...
mig_external kern_return_t catch_mach_exception_raise_state(
    ...
    if (!flavor || !new_stateCnt) {
        fmt::print(stderr, "umbra: catch_mach_exception_raise_state: Invalid arguments.\n");
        return KERN_INVALID_ARGUMENT;
    }
    if (*flavor != THREAD_STATE || old_stateCnt != THREAD_STATE_COUNT || *new_stateCnt < THREAD_STATE_COUNT) {
        fmt::print(stderr, "umbra: catch_mach_exception_raise_state: Unexpected flavor.\n");
        return KERN_INVALID_ARGUMENT;
    }
    if (exception != EXC_BAD_ACCESS) {
        fmt::print(stderr, "umbra: catch_mach_exception_raise_state: Unexpected exception type.\n");
        return KERN_FAILURE;
    }

    // The input/output pointers are not necessarily 8-byte aligned.
    umbra_thread_state_t ts;
